...
function createInitWindow(url) {
  ...
  shortcut.register(initWin, "Ctrl+L", () => {
    //prevent cache
    addQuery(()=>{
          switch (checkURL(url)) {
            case "social":
  ...
  shortcut.register(initWin, "F3", () => {
      ...
      initWin.webContents.executeJavaScript('pc.app.fire("Chat:Message", "Valid-Client", "Link copied!")').catch(e=>{});
  })
  ...
  initWin.webContents.on("dom-ready", (event) => {
    initWin.setTitle(`Valid-Client V${app.getVersion()}`);
    event.preventDefault();
  });
...
autoUpdater.on('update-available', (info) => {
    const dialogOpts = {
        type: 'info',
        buttons: ['Alright!'],
        title: 'MVP-Client Update',
        message: 'New Version of MVP-Client has been released',
